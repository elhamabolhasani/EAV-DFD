...
from models.vocalist.voca_model.transformer_encoder import TransformerEncoder
from models.vocalist.voca_model.conv import Conv2d, Conv3d
...
class SyncTransformer(nn.Module):
    ...
    def forward(self, frame_seq, mel_seq):
        ...
        """
        av_embedding, av_emb_list, av_qk, av_q_norm, av_k, av_v_norm, av_v = self.av_transformer(aud_embedding,
                                                                                                 vid_embedding,
                                                                                                 vid_embedding)
        va_embedding, va_emb_list, va_qk, va_q_norm, va_k, va_v_norm, va_v, = self.va_transformer(vid_embedding,
                                                                                                  aud_embedding,
                                                                                                  aud_embedding)
        self.feature_list['AV_Trans'] = {'av_emb': av_embedding, 'va_emb': va_embedding,
                                         'av_emb_list': av_emb_list, 'va_emb_list': va_emb_list, 'av_qk': av_qk,
                                         'va_qk': va_qk,
        ...
        t: 80 """
        tranformer_out, fus_emb_list, men_qk, mem_q_norm, mem_k, men_v_norm, men_v = self.mem_transformer(av_embedding,
                                                                                                          va_embedding,
                                                                                                          va_embedding)
        ...
        return logits_clsf.squeeze(-1), h_pooled, self.feature_list
...
if __name__ == "__main__":
    ...
    # Total trainable params 13.28 M
    # vid_prenet trainable params 6.69 M
    # aud_prenet trainable params 0.76 M
    # av_transformer trainable params 1.93 M
    # va_transformer trainable params 1.93 M
    # mem_transformer trainable params 1.93 M
    # fc trainable params 0.04 M

    # print(f"vid_prenet: {model.vid_prenet}")
    print(
        'Total trainable params {:.2f} M'.format((sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6)))
    print('vid_prenet trainable params {:.2f} M'.format(
        (sum(p.numel() for p in model.vid_prenet.parameters() if p.requires_grad) / 1e6)))
    print('aud_prenet trainable params {:.2f} M'.format(
        (sum(p.numel() for p in model.aud_prenet.parameters() if p.requires_grad) / 1e6)))
    print('av_transformer trainable params {:.2f} M'.format(
        (sum(p.numel() for p in model.av_transformer.parameters() if p.requires_grad) / 1e6)))
    print('va_transformer trainable params {:.2f} M'.format(
        (sum(p.numel() for p in model.va_transformer.parameters() if p.requires_grad) / 1e6)))
    print('mem_transformer trainable params {:.2f} M'.format(
        (sum(p.numel() for p in model.mem_transformer.parameters() if p.requires_grad) / 1e6)))
    print(
        'fc trainable params {:.2f} M'.format((sum(p.numel() for p in model.fc.parameters() if p.requires_grad) / 1e6)))
    ...
    print(f"Fus_Trans mem_qk: {stu_feature_list['Fus_Trans']['mem_qk'][3].shape}")
    print(f"Fus_Trans mem_v_norm: {stu_feature_list['Fus_Trans']['mem_v_norm'][3].shape}")
    print(f"Fus_Trans men_v: {stu_feature_list['Fus_Trans']['men_v'][3].shape}")