...
class HubertModel(nn.Module):
    ...
    def forward(self, input_values, attention_mask=None):
        ...
        # x = self.transformer_model(input_values, attention_mask=attention_mask , output_hidden_states
        # =True).hidden_states[12]
        ...
        x = self.merge(x)
        # run the code below to :  create a single vector as the representation of the entire input audio by passing
        # all embedding vectors to a recurrent network and extracting the last hidden state x,
        # _ = self.recurent_layer(x) x = x[:,-1]
...
def get_model(check_point, num_classes, device):
    model = HubertModel(check_point, num_classes)
