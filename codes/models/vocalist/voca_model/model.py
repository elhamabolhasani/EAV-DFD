...
class SyncTransformer(nn.Module):
    ...
    def forward(self, frame_seq, mel_seq):
        ...
        tranformer_out, fus_emb_list, men_qk, mem_q_norm, mem_k, men_v_norm, men_v = self.mem_transformer(
            av_embedding, va_embedding, va_embedding)
...
if __name__ == "__main__":
    ...
    output, h_pooled,  stu_feature_list = model(frame_seq, mel_seq)
    # print(f"output: {output.shape}, stu_feature_list: {stu_feature_list['CNN_feature']['aud'].shape}")

    print(f"output: {output.shape}")
    # print(f"AV_Trans mem_emb: {stu_feature_list['AV_Trans']['av_emb'].shape}")
    print(len(stu_feature_list['AV_Trans']['av_emb_list']))
    print(stu_feature_list['AV_Trans']['av_emb_list'][0].shape)
    print(len(stu_feature_list['AV_Trans']['va_emb_list']))
    print(stu_feature_list['AV_Trans']['va_emb_list'][0].shape)

    # print(f"AV_Trans mem_qk: {torch.Tensor(stu_feature_list['AV_Trans']['av_emb_list']).shape}")
    # print(f"AV_Trans mem_v_norm: {stu_feature_list['AV_Trans']['va_emb'].shape}")
    # print(f"AV_Trans men_v: {torch.Tensor(stu_feature_list['AV_Trans']['va_emb_list']).shape}")