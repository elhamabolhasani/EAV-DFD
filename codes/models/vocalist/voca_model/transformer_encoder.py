...
from models.vocalist.voca_model.position_embedding import SinusoidalPositionalEmbedding
from models.vocalist.voca_model.multihead_attention import MultiheadAttention
import math
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        for layer in self.layers:
            ...
            inter_x.append(x)
            inter_sf_qk.append(softmax_qk)
            inter_q_norm.append(q_norm)
            inter_k.append(k)
            inter_v_norm.append(v_norm)
            inter_v.append(v)

        if self.normalize:
            x = self.layer_norm(x)
            inter_x = [self.layer_norm(_x) for _x in inter_x]

        return x, inter_x, inter_sf_qk, inter_q_norm, inter_k, inter_v_norm, inter_v
...
if __name__ == '__main__':
    ...
    x1, softmax_qk1, q1, k1, v_norm1, v1 = encoder(x)
    print(f"encoder(x) shape: {x1[-1].shape}, softmax_qk: {softmax_qk1[-1].shape}, q1: {q1[-1].shape}, "
          f"k1: {k1[-1].shape}, only v1: {v_norm1[-1].shape}")
